"""
DREAMS数据集批量睡眠分期测试 - 使用真正的ASR结果
对比：原始信号 vs 真正ASR去噪 vs 深度学习去噪
"""
import os
import numpy as np
import warnings
...
output_file = OUTPUT_DIR / "batch_staging_results_with_asr.txt"
...
# ================= 分期函数 =================
TARGET_SFREQ = 100  # DeepSleepNet使用100Hz
EPOCH_LEN = 3000    # 30秒 * 100Hz (DeepSleepNet期望的输入长度)
...
def run_deepsleepnet_staging(data, sfreq, model):
    ...
    # 确保形状正确: (n_epochs, 3000, 1)
    epochs = epochs.reshape(-1, EPOCH_LEN, 1)
    
    # 验证形状
    if epochs.shape[1] != EPOCH_LEN:
        print(f"  警告: epoch长度不正确 {epochs.shape}")
        return np.array([])
...
def load_asr_result(asr_path):
    """加载ASR处理结果（.set文件）"""
    try:
        # 使用mne加载EEGLAB的.set文件
        raw = mne.io.read_raw_eeglab(str(asr_path), preload=True, verbose=False)
        data = raw.get_data()[0]  # 获取第一个通道
...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    log("对比：原始信号 vs 真正ASR去噪 vs 深度学习去噪", f)
    log("=" * 80, f)
    ...
    for edf_file in edf_files:
        ...
        # 查找对应的ASR处理结果
        asr_path = asr_dir / f"{subject_id}_ASR.set"
        if not asr_path.exists():
            log(f"\n{subject_id}: 未找到ASR处理结果，跳过", f)
            continue
        ...
        try:
            ...
            log(f"  原始采样率: {sfreq_orig}Hz, 数据长度: {len(data_raw)/sfreq_orig:.1f}秒", f)
            
            # 加载ASR处理结果
            log(f"  加载ASR处理结果...", f)
            data_asr, sfreq_asr = load_asr_result(asr_path)
            if data_asr is None:
                log(f"  ASR加载失败，跳过", f)
                continue
            log(f"  ASR采样率: {sfreq_asr}Hz, 数据长度: {len(data_asr)/sfreq_asr:.1f}秒", f)
            ...
            if sfreq_orig != TARGET_SFREQ:
                n_samples = int(len(data_raw) * TARGET_SFREQ / sfreq_orig)
                data_raw = np.interp(np.linspace(0, len(data_raw), n_samples), np.arange(len(data_raw)), data_raw)
                log(f"  重采样后数据长度: {len(data_raw)} (采样率: {TARGET_SFREQ}Hz)", f)
            ...
            # 睡眠分期
            log(f"  进行睡眠分期...", f)
            pred_raw = run_deepsleepnet_staging(data_raw, TARGET_SFREQ, deepsleepnet)
            pred_asr = run_deepsleepnet_staging(data_asr, sfreq_asr, deepsleepnet)
            pred_dl = run_deepsleepnet_staging(data_dl, TARGET_SFREQ, deepsleepnet)
