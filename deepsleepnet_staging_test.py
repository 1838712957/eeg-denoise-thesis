...
output_file = Path(__file__).resolve().parent / "deepsleepnet_staging_result.txt"
...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    for raw_path in raw_files:
        ...
        try:
            ...
            L = min(len(y_true) * 256 * 30, len(data_raw), len(data_clean))
            h_raw = run_deepsleepnet_staging(data_raw[:L], 256, deepsleepnet)
            h_clean = run_deepsleepnet_staging(data_clean[:L], 256, deepsleepnet)
            ...
            stage_results_raw = calculate_per_stage_accuracy(y_true_final, h_raw_final, STAGE_NAMES)
            stage_results_clean = calculate_per_stage_accuracy(y_true_final, h_clean_final, STAGE_NAMES)
            
            log(f"\n{'='*80}", f)
            log(f"受试者: {subject_id}", f)
            log(f"{'='*80}", f)
            log(f"总体准确率 - 原始: {acc_raw:.2f}%, 去噪: {acc_clean:.2f}%, 变化: {acc_clean-acc_raw:+.2f}%", f)
            log(f"\n分期       | 原始准确率        | 去噪准确率       | 变化         | 样本数", f)
            log("-" * 60, f)
            for name in STAGE_NAMES:
                r_raw = stage_results_raw[name]
                r_clean = stage_results_clean[name]
                if r_raw['total'] > 0:
                    change = r_clean['accuracy'] - r_raw['accuracy']
                    log(f"{name:8} | {r_raw['accuracy']:6.2f}       % | {r_clean['accuracy']:6.2f}       % | {change:+.2f}% | {r_raw['total']}", f)
                    all_stage_acc_raw[name].append(r_raw['accuracy'])
                    all_stage_acc_clean[name].append(r_clean['accuracy'])
    ...
    log(f"{'='*80}", f)
    log(f"\n各分期平均准确率:", f)
    log(f"分期       | 原始准确率        | 去噪准确率       | 变化", f)
    log("-" * 50, f)
    for name in STAGE_NAMES:
        if all_stage_acc_raw[name]:
            avg_raw = np.mean(all_stage_acc_raw[name])
            avg_clean = np.mean(all_stage_acc_clean[name])
            log(f"{name:8} | {avg_raw:6.2f}       % | {avg_clean:6.2f}       % | {avg_clean-avg_raw:+.2f}%", f)
